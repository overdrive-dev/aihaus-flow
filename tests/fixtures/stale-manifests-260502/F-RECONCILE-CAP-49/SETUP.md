# Setup — F-RECONCILE-CAP-49

**Fixture type:** Manual integration test / future harness placeholder
**Boundary tested:** 49 commits ahead of integration ref → full `[CATEGORY B]` recipe path

## What this fixture tests

When a worktree has commits ahead of the closest integration ref that are at or below
`AIHAUS_RECONCILE_CAP` (default 50), `worktree-reconcile.sh` must emit a full
`[CATEGORY B]` cherry-pick recipe containing all commit SHAs.

This fixture documents the boundary at cap-1 (49 commits).
...
```bash
#!/usr/bin/env bash
# SETUP.sh — construct a stub git repo with 49 commits ahead of a stub integration ref.
set -euo pipefail
...
# Create a worktree branch with exactly 49 commits
...
# Expected stdout: one [CATEGORY B] block with 49 commits listed, no [INTEGRATION-LAG].
