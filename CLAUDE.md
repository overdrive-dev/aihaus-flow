...
**Cohort aliases** (v0.31.0 / M027 / ADR-260509-Y — 6→5 fork). All 59 agents are
...
confirmation) or `--agent <member>` can mutate it. Full 59-agent mapping + prose
...
declares two new YAML fields (59 agents classified; smoke-test Check 6 enforces both):
...
Since v0.36.0 / M041 (3 sub-stories S1/S2/S3, no new ADR — incremental atop ADR-260515-B-amend-02 substrate), aihaus collapses the aih-graph install dance from 3 commands to 2 and adds **pure-Go offline BM25/FTS5 lexical search**. S1: aih-graph introduced default BM25/FTS5 lexical rows; new `nodes_fts` virtual table (SQLite FTS5; ships in stock modernc/sqlite); `internal/storage/fts5.go` exposes `SaveFTS()` + `QueryFTS5()` using SQLite's built-in `bm25()` ranking function (no roll-own scoring). Empirical query quality on aihaus-flow's own ADRs (153 indexed nodes): query "merge-settings lida com hooks arrays" → ADR-260514-B ranks #1 by 2× margin; "sqlite vector pivot" → 4 data-model-pivot ADRs land in top 4. M048 later removes Voyage/fake/none provider selection entirely; semantic embeddings now mean local Ollama `nomic-embed-text` only. S2: `/aih-init` ganha Phase 3 — auto-dispatches `aih-graph build --accept-all-repos .` after `project.md` generation; 5-step flow lives in `aih-init/annexes/aih-graph-bootstrap.md` (binary discovery → consent marker → build → Ollama hint → completion summary); non-fatal envelope keeps `/aih-init` exit-0 regardless of Phase 3 outcome. S3: `pkg/scripts/install.sh` Step 13 auto-invokes `install-aih-graph-binary.sh` on initial install (skipped on `--update`); soft-fail on network / platform-unsupported / source-script-missing; `AIHAUS_SKIP_GRAPH_BINARY=1` opt-out for CI / airgapped. Net result: end-user flow is `bash install.sh --target .` then `/aih-init` — zero credit card, zero CGO toolchain. Smoke check enforcement-audit canonical (Check 62) extended from 337 → 343 rows mirroring the 6 new H2 sections in `aih-graph-bootstrap.md`. Binary size 10.7MB → 16.1MB (FTS5 + tokenizer code shipped with modernc/sqlite; no new deps). Tag pair this release: `aihaus-v0.36.0` + `aih-graph-v0.1.1`.

Since v0.35.0 / M033–M040 (ADR-260515-A through E + 8 amendments), aihaus ships **aih-graph v0.1.0** as the mandatory structural+semantic memory engine. Pure-Go standalone binary (10.7 MB; zero CGO; `modernc.org/sqlite` + roll-own Go KNN — converged after 3 design pivots: graphify dep → standalone Go binary → SQLite+sqlite-vec → pure-Go). Scope: **markdown-only structured extraction** for 6 aihaus typed nodes (`Decision`, `Milestone`, `Story`, `Agent`, `Hook`, `Skill`) from `pkg/.aihaus/{decisions.md, milestones/, agents/, hooks/, skills/}`. **3 query modes** in `cmd/aih-graph`: `--bfs` (recursive CTE over `edges` table, structural-only), `--semantic` (Ollama vector KNN with BM25 fallback), `--hybrid` (KNN top-K + 1-hop edge expansion per match, or BM25 fallback). **Embedding pipeline:** fixed local Ollama `nomic-embed-text`; BM25 rows are always refreshed and vector rows are added when Ollama is available. SHA-based change detection (`content_sha` column) skips re-embedding unchanged nodes. **Privacy contract per ADR-260515-A** (`internal/privacy/`): per-repo `.db` file under XDG state root (`$XDG_STATE_HOME/aih-graph/<sha256-hex-16-of-abs-repo-path>/graph.db` on Linux; `~/Library/Application Support/aih-graph/<hash>/graph.db` on macOS; `%LOCALAPPDATA%/aih-graph/<hash>/graph.db` on Windows; `AIH_GRAPH_HOME` env var overrides). `aih-graph build` refuses without `.aih-graph-consent` marker at repo root unless `--accept-all-repos` (creates marker). `aih-graph uninstall <repo>` purges one repo's `.db` + WAL/SHM sidecars; `aih-graph uninstall --purge` removes entire state root. **Integration touchpoints (M039/M048):** `pkg/scripts/install-aih-graph-binary.sh` downloads platform-matched binary; `pkg/.aihaus/hooks/aih-graph-refresh.sh` on-demand index refresh with 4-tier binary discovery (`AIH_GRAPH_BIN` env > `~/.aihaus/bin/aih-graph[.exe]` > `$CLAUDE_PROJECT_DIR/aih-graph/bin/` > `PATH`); `context-inject.sh` auto-injects a bounded native repository memory packet on SubagentStart, and 59/59 packaged agents use `aihaus memory ... --json` for deeper agent-specific context. `aih-graph-refresh.sh` opportunistically starts local Ollama and otherwise leaves the BM25 index fresh. **CI / release workflows** `.github/workflows/aih-graph-{ci,release}.yml` cross-compile 4-platform matrix via `actions/setup-go@v5` + `CGO_ENABLED=0 go build`; release workflow triggers on tag pattern `aih-graph-v*` → uploads stripped binaries (`-trimpath -ldflags="-s -w"`) + per-platform SHA-256 checksums. **Smoke checks 84/85/86/87** cover ADR forcing functions, build smoke, integration round-trip, and M048 hook/agent memory contracts. Hook count: 32 → 33 (`aih-graph-refresh.sh` + `tdd-guard.sh` allowlist correction). New env vars (M039/M048): `AIH_GRAPH_HOME`, `AIH_GRAPH_BIN`, `AIH_GRAPH_VERSION`, `AIH_GRAPH_REPO`, `AIH_GRAPH_DB`, `AIH_GRAPH_QUIET`, `AIHAUS_OLLAMA_AUTO`, `AIH_GRAPH_OLLAMA_URL`, `AIHAUS_MEMORY_INJECT`, `AIHAUS_MEMORY_CONTEXT_MAX_BYTES`, `AIHAUS_MEMORY_QUERY_TOP`. **Deferred:** local ONNX embeddings and public `pkg/aihgraph/` Go library API.
...
- `effort: <tier>` frontmatter on all 59 agents → native CC field; the M008 cohort taxonomy (`:planner-binding`, `:planner`, `:doer`, `:verifier`, `:adversarial`) composes byte-identically with native semantics. Smoke Check 6 enforces.
...
- **Contract + ledger** — `pkg/.aihaus/protocols/business-rules.md` (spec: autonomy law, rule
...
- **Determinism hooks** — `flow-guard.sh` (PreToolUse, BR-F2): blocks an online-action/deploy
  ...
  on `--to running`, clears on `complete|aborted`); the sole online-boundary gate
  (ADR-260612-A); deploy patterns are a single shared source `hooks/lib/online-actions.sh`. The **rule-gate** (`calibrate-guard.sh`)
...
S02 stale cohort-defaults.json resilience); Check 102 — context-inject harness+receipt fixture-fail
