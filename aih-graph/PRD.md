...
- **M035 — Query + typed accessors + embedding pipeline:** `internal/query/` (recursive CTE BFS for structural; Go-side KNN for semantic; hybrid composition) + `internal/types/` (6 typed accessor structs: `Decision`, `Milestone`, `Story`, `Agent`, `Hook`, `Skill` — each with Get/Find/List methods over SQL) + `internal/embed/knn.go` (~100 LOC pure-Go brute-force cosine; goroutine fan-out optional for >50k vectors) + `internal/embed/embed.go` (single Ollama `nomic-embed-text` embedder; local-ONNX deferred indefinitely) + `pkg/aihgraph/` public API.
- **M036 — Privacy gates:** `internal/privacy/` (XDG resolution: `$XDG_STATE_HOME/aih-graph/<repo-hash>/graph.db` or platform equivalent + per-repo file isolation + consent gate via `.aih-graph-consent` marker file + `--purge` = single file delete).
...
- [ ] `aih-graph query --semantic --json "..."` returns semantic matches when Ollama embeddings exist, and BM25-backed matches otherwise.
...
- [ ] `aih-graph build` always writes BM25 rows and opportunistically enriches rows with local Ollama embeddings.
