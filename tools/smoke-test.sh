...
# ---- Check 3: .aihaus/hooks/ has 27 .sh files (M017/S03 adds merge-back.sh) ----
check_hooks() {
  _start_check
  local label="Check ${CHECK_NUMBER}: .aihaus/hooks/ has 27 .sh files"
  ...
  if [[ "$count" -eq 27 ]]; then
    ...
    _fail "$label" "expected 27 .sh files, found $count"
  fi
}
