...
# ---- Check 3: .aihaus/hooks/ has 25 .sh files (M017/S02b adds worktree-release-all.sh) ----
check_hooks() {
  _start_check
  local label="Check ${CHECK_NUMBER}: .aihaus/hooks/ has 25 .sh files"
  ...
  if [[ "$count" -eq 25 ]]; then
    ...
    _fail "$label" "expected 25 .sh files, found $count"
  fi
}
