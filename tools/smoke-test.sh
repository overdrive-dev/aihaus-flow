...
# ---- Check 3: .aihaus/hooks/ has 24 .sh files (M017/S02a adds worktree-release.sh) ----
check_hooks() {
  _start_check
  local label="Check ${CHECK_NUMBER}: .aihaus/hooks/ has 24 .sh files"
  ...
  if [[ "$count" -eq 24 ]]; then
    ...
    _fail "$label" "expected 24 .sh files, found $count"
  fi
}
