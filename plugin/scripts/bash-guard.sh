...
if echo "$COMMAND" | grep -qiE 'rm\s+-rf\s+(/|~|\$HOME|C:[\\])|git\s+push\s+--force\s+(origin\s+)?(main|master|staging|production)|drop\s+(table|database)\s|truncate\s+|git\s+clean\s+-fd|mkfs\.|dd\s+if='; then
