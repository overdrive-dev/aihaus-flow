...
if [[ -f "${CLAUDE_DIR}/CLAUDE.md" ]]; then
  ...
  while IFS= read -r import_line; do
    ...
    elif [[ "${rel}" == "../.aihaus/memory/local/user-preferences-global.md" ]]; then
      # M050/S06 (ADR-260611-E §4): the tier-C mirror is regenerated by
      # project-context-refresh.sh only when ~/.aihaus/memory/user/preferences.md
      # exists. Absence is by-design (no global preferences yet), not a broken
      # import — Claude Code tolerates missing @import targets.
      add_row "OK" "Import ${import_line}" "optional tier-C mirror absent (no global preferences yet)"
    else
