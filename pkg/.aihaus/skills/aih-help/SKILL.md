...
| `/aih-goal [description or list] [--run-to-completion] [--until human-review]` | Discover planned kanban tasks or import a pasted/local list, register planning Q/A contracts, and execute ready work without mid-run input | You have a batch of planned tasks and want them advanced until a target workflow stage |
...
/aih-goal --from-list --run-to-completion --until human-review
  -> keeps advancing backlog/list tasks until the target stage or a true blocker
```
