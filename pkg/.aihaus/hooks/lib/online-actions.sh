...
#
# Sourced by BOTH role-guard.sh (role × online boundary) and flow-guard.sh
# (flow × online boundary), so the deploy-detection list has a SINGLE definition.
# This is the M030 drift lesson applied: one canonical source, never two copies
...
aihaus_online_action_regex() {
  ...
  local conf="${root}/.aihaus/roles/online-actions.conf"
  if [ -f "$conf" ]; then
